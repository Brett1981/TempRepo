...
{
    /// <summary>
    /// Posts Sales Payments to Sage 200 (POST /sales_payments).
    /// Builds snake_case body and NEVER sends explicit nulls (standing rule).
    /// </summary>
    public sealed class SalesPaymentsService : ISalesPaymentsService
    {
        private readonly ISageApiClient _sage;
        private readonly ILogger<SalesPaymentsService> _log;
        private readonly IEventPublisher? _events; // optional Kafka

        public SalesPaymentsService(
            ISageApiClient sage,
            ILogger<SalesPaymentsService> log,
        ...
        /// <inheritdoc />
        public async Task<SalesCreateResult> CreateAsync(SalesPaymentCreate request, HttpContext http, CancellationToken ct)
        {
            // ---------- Header guard ----------
            ...
            // ---------- Body (omit-nulls) ----------
            var body = BuildPaymentBody(request);
            ...
            // ---------- POST upstream ----------
            var (status, bodyText) = await _sage.PostForBodyAsync("sales_payments", body, headers, ct);

            if (status is < 200 or > 299)
            {
                _log.LogWarning("Upstream error creating SalesPayment: status={Status} body={Body}",
            ...
            _log.LogInformation("sales_payment created with URN={Urn}", urn);
            ...
            try
            {
                ...
                {
                    await _events.PublishAsync("sales.payment.created", new
                    {
                        urn,
            ...
            {
                _log.LogWarning(ex, "Kafka publish failed for SalesPayment URN={Urn}", urn);
            }
        ...
        private static Dictionary<string, object> BuildPaymentBody(SalesPaymentCreate req)
        {
            var body = new Dictionary<string, object>();

            void Add<T>(string name, T? value) where T : struct
            {
                if (!EqualityComparer<T?>.Default.Equals(value, default)) body[name] = value!;
            }
